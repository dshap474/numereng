...
Do not use this skill for research strategy, plateau decisions, or sweep methodology; use `experiment-design`. Do not use it for drift repair, reindex, destructive cleanup, or reset workflows; use `utility-store-ops`.
...
- Repo-internal sources, only when editing numereng itself:
  - `src/numereng/config/training/AGENTS.md`
...
- store drift has been handed off to `utility-store-ops` when normal experiment commands are no longer enough

## Store Handoff

The store DB at `.numereng/numereng.db` should stay aligned with run directories and experiment manifests.

Use `utility-store-ops` when the task becomes maintenance or repair, especially:
...
If `doctor` reports one known run missing from the index, use `numereng store index --run-id <run_id>`. For broad drift or suspected corruption, use `numereng store rebuild` or hand off to `utility-store-ops`.
