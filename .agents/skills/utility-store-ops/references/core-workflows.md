...
uv run python .agents/skills/utility-store-ops/scripts/collect_store_impact.py \
...
```bash
uv run python .agents/skills/utility-store-ops/scripts/reset_runs.py \
  --run-id <run_id>
```
...
```bash
uv run python .agents/skills/utility-store-ops/scripts/reset_experiment_runs.py \
  --experiment-id <experiment_id>
```
...
```bash
bash .agents/skills/utility-store-ops/assets/bash/verification-checks.sh <experiment_id> [<experiment_id> ...]
```
...
```bash
sqlite3 .numereng/numereng.db < .agents/skills/utility-store-ops/assets/sql/impact-queries.sql
```
