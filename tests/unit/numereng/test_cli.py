...
def test_cli_main_translates_missing_runtime_dependency(
    ...
    def fake_import(
        ...
        if name == "numereng.cli.commands.serve":
            raise ModuleNotFoundError("No module named 'cloudpickle'", name="cloudpickle")
        return real_import(name, globals, locals, fromlist, level)

    main_module = importlib.import_module("numereng.cli.main")
    monkeypatch.setattr(main_module, "__import__", fake_import, raising=False)
...
def test_cli_research_commands_success(
    ...
    monkeypatch.setattr(
        ...
        lambda request: api_module.ResearchStatusResponse(
            ...
            ledger_path="/tmp/agentic_research/ledger.jsonl",
        ...
        lambda request: api_module.ResearchRunResponse(
            ...
            rounds=[
                api_module.ResearchRoundResponse(
                    ...
                    action="run",
                    status="completed",
                    config_path="/tmp/configs/r002.json",
                    run_id="run-5",
                    metric_value=0.124,
    ...
    assert exit_code == 0
    assert "r002 | run | run_id=run-5 | metric=0.124000" in capsys.readouterr().out
