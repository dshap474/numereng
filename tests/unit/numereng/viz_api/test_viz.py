...
def test_get_submission_calibration_reads_materialized_artifacts(tmp_path: Path) -> None:
        ...
        [
            {
                ...
                "local_bmc200_mean": 0.12,
                "live_mmc20": -0.005,
                "scored_round_count": 1,
                "has_live_score": True,
            }
        ]
    ).to_parquet(artifact_root / "calibration_observations.parquet", index=False)
    (artifact_root / "report.json").write_text(
        json.dumps({"scopes": {"all_scored": {"row_count": 1, "observation_count": 1}}}),
        encoding="utf-8",
    )
    (artifact_root / "manifest.json").write_text(
        json.dumps({"row_count": 1, "observation_count": 1}),
        encoding="utf-8",
    )
    adapter = VizStoreAdapter(VizStoreConfig(store_root=store_root, repo_root=tmp_path))
    ...
    assert payload["total"] == 1
    assert payload["row_total"] == 1
    assert payload["observations"][0]["model_name"] == "model_a"
    assert payload["observations"][0]["upload_id"] == "upload-a"
