...
def test_initialize_run_log_truncates_existing_file(tmp_path: Path) -> None:
...
def test_run_log_helpers_append_structured_lines(tmp_path: Path) -> None:
    ...
    assert len(lines) == 3
    assert "INFO | run_started | run_id=abc123" in lines[0]
    assert "INFO | stage_update | load_data :: Loading data." in lines[1]
    assert "ERROR | run_failed | boom" in lines[2]
