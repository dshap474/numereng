...
1. Start the dashboard with `numereng viz` from the workspace root. If you are debugging the source checkout itself, `just dev` remains a contributor wrapper.
2. Confirm the API health check: `http://127.0.0.1:8502/healthz`.
3. Inspect the backend stdout/stderr for the running `numereng viz` process. If you launched via `just dev`, inspect `viz/api.log` and `viz/vite.log`.
