...
| `.agents/` and `.claude/` | Tracked | Agent skills library (`.agents/skills/`, packaged subset in `SHIPPED.txt`) and the `.claude/skills` symlink into it | Audit as normal tracked content; `.claude/settings.local.json` stays gitignored |
