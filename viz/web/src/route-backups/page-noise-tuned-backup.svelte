<script lang="ts">
	...
	onMount(() => {
		...
		const resize = () => {
			...
			noiseCanvas.width = Math.max(1, Math.floor(terminalEl.clientWidth * dpr));
			noiseCanvas.height = Math.max(1, Math.floor(terminalEl.clientHeight * dpr));
		...
		const fragmentSource = `
			...
			void main() {
				vec2 s = gl_FragCoord.xy / res.xy;
				s *= vec2(res.x, res.y) * 1.5;
				float raw = rnd(s + t * 0.001);
				float signedNoise = raw - 0.5;
				float alpha = smoothstep(0.46, 0.5, abs(signedNoise)) * 0.003;
				vec3 color = signedNoise > 0.0 ? vec3(0.82) : vec3(0.08);
		...
		<section class="full-bleed-graphic">
			<svg class="sun-core-svg" viewBox="0 0 400 400" aria-hidden="true">
				...
				<g>
					...
					<text x="290" y="125" fill="var(--ink)" font-family="Space Mono" font-size="8">
						CORR_NATIVE
					</text>
					<circle cx="100" cy="300" r="3" fill="var(--ink)"></circle>
					<text x="60" y="305" fill="var(--ink)" font-family="Space Mono" font-size="8">
						FNC_MEAN
					</text>
				</g>
			</svg>
			...
			<div class="overlay-data">
				<div class="readout-box">
					<div class="sys-label">BMC_LAST_200</div>
					<div class="readout-val">0.1042</div>
				</div>
				<div style="align-self: flex-end">
					<div class="readout-box" style="margin-bottom: 8px">
						<div class="sys-label">MMC_MEAN</div>
						<div class="readout-val">0.0187</div>
					</div>
					<div
						class="capsule"
...
<style>
	...
	.full-bleed-graphic {
		...
		align-items: center;
		background: radial-gradient(circle, rgba(214, 104, 139, 0.05) 0%, transparent 70%);
		z-index: 3;
	}
