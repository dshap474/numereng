	...
	>
		<div class="flex min-h-0 flex-1 flex-col px-4 pb-4 pt-[20px]">
			...
			<div class="min-h-0 flex-1 overflow-y-auto">
				...
				<div class="mt-1 group relative">
					...
					{#if showExperimentList && data.experiments.length > 0}
						<div class="shell-submenu max-h-[60vh] overflow-y-auto">
									...
									{#each data.experiments as exp (exp.experiment_id)}
										<li>
											<a
												...
												class="shell-submenu-link block px-3 py-3 {isActive(exp)
													? 'shell-submenu-link-active'
													: ''}"
												onclick={closeSidebar}
											>
												<div class="flex items-center justify-between mb-1">
													<span class="text-[9px] text-reader-sidebar-muted/70 tabular-nums">{exp.created_at?.slice(0, 10) ?? ''}</span>
													</div>
			...
			<div class="hidden pt-3 md:block">
				<button
					type="button"
					class="shell-collapse-button flex w-full items-center rounded-md py-2 text-sm justify-center px-0"
					aria-label={sidebarCollapsed ? 'Expand sidebar' : 'Collapse sidebar'}
					onclick={toggleSidebarCollapsed}
				>
					<svg class="reader-chevron h-4 w-4 flex-shrink-0 {sidebarCollapsed ? 'rotate-180' : ''}" fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="1.8">
