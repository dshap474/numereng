<script lang="ts">
	...
	let showDocsList = $derived(docsExpanded && !sidebarCollapsed);
	let showExperimentList = $derived(experimentsExpanded && !sidebarCollapsed);
...
<div class="flex h-screen">
	...
	<aside
		class="w-60 border-r border-sidebar-border bg-sidebar-background flex flex-col flex-shrink-0 h-screen fixed z-30 transition-transform md:relative md:translate-x-0 md:overflow-hidden md:transition-[width] md:duration-200 {sidebarOpen ? 'translate-x-0' : '-translate-x-full'} {sidebarCollapsed ? 'md:w-[4.75rem]' : 'md:w-60'}"
	>
		<div class="flex min-h-0 flex-1 flex-col px-4 pb-4 pt-[20px]">
			<a
				href="/"
				class="mb-2 block rounded-md text-lg font-semibold tracking-tight text-sidebar-primary transition-all hover:text-white active:scale-[0.97] {sidebarCollapsed ? 'px-0 text-center' : 'px-3'}"
				aria-label="Numereng"
				onclick={closeSidebar}
			>
				{sidebarCollapsed ? 'N' : 'Numereng'}
			</a>
				...
				<div class="mt-1 group relative">
					<div
						class="flex items-center gap-2 px-3 py-2 text-sm transition-colors {showDocsList ? 'rounded-t-md bg-sidebar-accent/30' : 'rounded-md'} {onDocsRoute
							...
							: 'text-sidebar-foreground hover:bg-sidebar-accent hover:text-sidebar-accent-foreground'} {sidebarCollapsed ? 'justify-center px-0' : ''}"
					>
						<a
							href="/docs"
						...
						{#if !sidebarCollapsed}
							<button
								...
								class="ml-auto rounded p-0.5 opacity-60 transition-all hover:bg-sidebar-border hover:opacity-100"
			...
			<div class="hidden pt-3 md:block">
				<button
					type="button"
					class="flex w-full items-center rounded-md py-2 text-sm text-sidebar-foreground transition-colors hover:bg-sidebar-accent hover:text-sidebar-accent-foreground justify-center px-0"
					aria-label={sidebarCollapsed ? 'Expand sidebar' : 'Collapse sidebar'}
					onclick={toggleSidebarCollapsed}
				>
					<svg class="h-4 w-4 flex-shrink-0 transition-transform {sidebarCollapsed ? 'rotate-180' : ''}" fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="1.8">
